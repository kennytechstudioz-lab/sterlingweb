...
export default function TermsPage() {
  ...
  useEffect(() => {
    apiCall<{ success: boolean; terms: TermSection[] }>("/api/terms")
      .then((res) => {
        setSections((res.terms || []).filter((t) => t.category === "terms"));
      })
      .catch(() => {})
          ...
          <div className="bg-white border border-neutral-200/50 rounded-lg p-8 sm:p-12 shadow-sm flex flex-col gap-10">
            ...
            {loading ? (
              <div className="flex items-center gap-3 py-12 justify-center">
                ...
                <span className="text-sm text-neutral-400">Loading terms...</span>
              </div>
            ) : sections.length === 0 ? (
              <p className="text-sm text-neutral-400 text-center py-10">No terms available at this time.</p>
            ) : (
