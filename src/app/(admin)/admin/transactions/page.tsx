...
export default function AdminTransactionsPage() {
  ...
  const fetchTransactions = async (page: number) => {
    ...
    try {
      const response = await apiCall<{
        ...
        totalPages: number;
      }>(`/api/users/transactions/all?page=${page}&limit=20`);
      setTransactions(response.transactions || []);
      setTotalPages(response.totalPages || 1);
    } catch (err: any) {
      setError(err.message || "Failed to retrieve system-wide transaction records.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const requestDeleteTransaction = (id: string) => {
    setToastConfirm({
      show: true,
      message: "Are you sure you want to delete this transaction record from the system permanently? This action cannot be undone.",
      type: "delete",
      id,
    });
  };
  ...
  const requestApproveTransaction = (id: string) => {
    setToastConfirm({
      show: true,
      message: "Are you sure you want to approve this pending transaction? This action will credit the user's wallet balance, allocate carbon assets, and start active compounding tranches immediately.",
      type: "approve",
      id,
    });
  };

  const proceedApproveTransaction = async (id: string) => {
    try {
      await apiCall(`/api/users/transactions/${id}/status`, {
        method: "PATCH",
        body: { status: "completed" },
      });
      // Locally transition status to completed
      setTransactions((prev) =>
        prev.map((t) => (t._id === id ? { ...t, status: "completed" as const } : t))
      );
    } catch (err: any) {
  ...
  // Filter transactions within current page
  const filteredTransactions = transactions.filter((t) => {
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    return (
      t.username.toLowerCase().includes(q) ||
      t.currencySymbol.toLowerCase().includes(q) ||
      t.currencyName.toLowerCase().includes(q) ||
    ...
    <div className="p-[10px] md:p-8 flex flex-col gap-6 w-full text-left min-h-screen pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-900 pb-5">
        <div className="flex flex-col gap-1">
          <h3 className="text-2xl font-black text-white tracking-tight uppercase">System Transactions</h3>
          <p className="text-sm text-neutral-400 font-light">
            Monitor and audit all investor ledger balances, deposits, withdrawals, and referrals system-wide.
          </p>
        </div>
        ...
        <div className="relative min-w-[280px]">
          <input
            type="text"
            placeholder="Search by username, currency, type..."
            value={searchQuery}
      ...
      <div className="bg-[#0f1115] border border-neutral-900 rounded-lg overflow-hidden flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8">
            ...
            <span className="text-sm text-white font-black uppercase tracking-wider">No matching transactions</span>
            <span className="text-xs text-neutral-500 mt-2">Try adjusting your query filter.</span>
          </div>
        ) : (
          <>
                    ...
                    <tr key={tx._id} className="hover:bg-neutral-900/20 transition-colors group">
                      ...
                      <td className="py-4 px-6 text-center">
                        <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          {/* Render custom approve button if status is pending */}
                          {tx.status === "pending" && (
                            <button
                              onClick={() => requestApproveTransaction(tx._id)}
                              className="w-full bg-[#122319]/80 hover:bg-[#122319] text-[#4ade80] border border-[#1e3a29] text-[9px] font-black uppercase tracking-wider py-1.5 px-3 rounded cursor-pointer transition-colors shadow-sm"
                            >
                              Approve
                            </button>
                          )}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-all duration-300">
          <div className="bg-[#0f1115] border-2 border-[#e4c126]/60 rounded-xl max-w-md w-full p-6 md:p-8 flex flex-col gap-6 shadow-[0_0_50px_rgba(228,193,38,0.15)] transform scale-100 transition-transform">
            <div className="flex items-center gap-3 border-b border-neutral-900 pb-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${toastConfirm.type === 'delete' ? 'bg-[#241315] text-[#f87171] border border-[#3d1e22]' : 'bg-[#122319] text-[#4ade80] border border-[#1e3a29]'}`}>
                {toastConfirm.type === 'delete' ? (
              ...
              <h4 className="text-lg font-black text-white uppercase tracking-wider">{toastConfirm.type === 'delete' ? 'Delete Ledger Record' : 'Confirm Action'}</h4>
            </div>
            ...
            <div className="flex items-center justify-end gap-3.5 mt-2">
              ...
              <button
                onClick={async () => {
                  const { type, id } = toastConfirm;
                  setToastConfirm(null);
                  if (type === 'delete') {
                    await proceedDeleteTransaction(id);
                  } else if (type === 'approve') {
                    await proceedApproveTransaction(id);
                  }
                }}
                className={`px-5 py-2 rounded text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${toastConfirm.type === 'delete' ? 'bg-red-600 hover:bg-red-500 text-white' : 'bg-[#e4c126] hover:bg-[#d8b520] text-neutral-900'}`}
              >
                Confirm and Proceed
              </button>
            </div>
